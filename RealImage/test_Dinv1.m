...
%         Bidx = lsqminnorm(RLs,y);

%         if idx == 1 
%             Bidx = inv(RLs.' * RLs + lambda2 * eye(4)) * RLs.' * y;
%         else
%             %Bidx = RLs\y;
%             Bidx = (RLs + randn(size(RLs))/(sqrt(size(RLs,1))+sqrt(size(RLs,2)))*1e-2 )\y;
%         end
...
%         cvx_end 
    ...
    sigma = 1 * .99^iter;
    sigma = max(1 * .95^iter, 0.05);
    Xhat = getX(Ahat, Y, sigma, p);
    fprintf("          target is %.2f\n",norm(Y - kron(Dinv2, Dinv1) * Xhat, 'fro'))

    diagnostic(iter,:)=[iter cond(full(Binv1{1})) cond(full(Binv1{2})) cond(full(Binv1{3})) norm(Xhat) mean((norms(Xhat,1).^2./norms(Xhat,2).^2))]
...
% idd = 55
% image = Dinv1 * image_gray{idd} * Dinv2.';
% s = floor(get_sparsity(image)) + 1;
% x = (sort(image(:),'descend'));
% threshold = abs(x(s));
% [mm,nn] = find(abs(image)<threshold);
% for i = 1:size(mm,1)
%     r = mm(i);
%     c = nn(i);
%     image(r,c) = 0;
