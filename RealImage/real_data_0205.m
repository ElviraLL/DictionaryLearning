...
for i = 1:p
    Xhat_image{i} = reshape(Xhat(:,i), N, N);
    Yhat{i} = Dinv1 * Xhat_image{i} * Dinv2.';
end
...
% end
% idd = 3
% Xest = Dinv1 * image_gray{idd} * Dinv2;
% [u,s,v] = svd(Xest);
% r = 5
% Xcut = u(:, 1:r) * s(1:r,1:r) * v(:,1:r)';
% image_recover = inv(Dinv1) * Xcut * inv(Dinv2);
% 
% figure
% subplot(1,2,1)
% imagesc(abs(image_recover));
% 
% subplot(1,2,2)
